...
export default {
  ...
  methods: {
    ...
    send_delete() {
      this.$refs.mo_advertencia_eliim.hide();
      API
        .post('controldocumentosvehiculos/nuevo', 
        { 
    ...
    send_descarga() {
      REAPI
        .post('controldocumentosvehiculoscsv')
        .then((resp) => {
    ...
    load_edit() {
      API
      .post("controldocumentosvehiculos/vista",
        {
...
<div class="main-container">
  ...
  <el-table :cell-class-name="cellStyle2" :data="datap" border header-row-style="color:black;" height="98%" :size="$isMobile() ? 'small':'default'">
      ...
      <el-table-column  :fixed="!$isMobile()" prop="vcl_nombre" label="Tipo " width="100" />
